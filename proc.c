...
int setrunningticks(int running_thresh){
  cprintf("old %d\n", RUNNING_THRESHOLD);
  RUNNING_THRESHOLD = running_thresh;
  cprintf("new %d\n", RUNNING_THRESHOLD);
  if(RUNNING_THRESHOLD == running_thresh)
    return 1;
  else
    return 0;
}

int setwaitingticks(int wait_thresh){
  cprintf("old %d\n", WAITING_THRESHOLD);
  WAITING_THRESHOLD = wait_thresh;
  cprintf("old %d\n", WAITING_THRESHOLD);
  if(WAITING_THRESHOLD == wait_thresh)
    return 1;
  else
    return 0;
}
...
{
  struct proc *p;
  int ran = 1; // CS550: to solve the 100%-CPU-utilization-when-idling problem
